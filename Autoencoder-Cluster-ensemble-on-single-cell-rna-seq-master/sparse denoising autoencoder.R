# Sparse denoising Autoencoder model implementation in keras
...
encode = function(dat, seed = 1, max_random_projection = 2048, encoded_dim = 16, hidden_dims = c(128), learning_rate = 0.001, batch_size = 32, epochs = 100, verbose = 2, scale = FALSE, genes_as_rows = FALSE) {
  ...
  for (layer_width in hidden_dims) {
    tns = keras::layer_dense(tns, units = layer_width, activity_regularizer = regularizer_l1(10e-5))
...
ensemble_cluster = function(dat, seed = 1, cluster_func = function(x) kmeans(x, centers=9), ensemble_sizes = c(1, 5, 10, 20, 50), cores = 1) {
...
pbmc_tr<-t(pbmc_counts)
sparse<-ensemble_cluster(pbmc_tr)

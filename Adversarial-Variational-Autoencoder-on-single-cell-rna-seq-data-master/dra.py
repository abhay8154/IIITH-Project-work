...
class Test_DRA(object):
    ...
    def build_model(self):
        ...
        self.kl_scale = tf.compat.v1.placeholder(tf.float32, (), name='kl_scale')
        ...
        self.proj = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.X_dim], name='projection')
        ...
        self.library_size_mean = tf.cast(tf.constant(library_size_mean), dtype=tf.float32)
        self.library_size_variance = tf.cast(tf.constant(library_size_variance), dtype=tf.float32)        
        ...
        kl_gauss_l = 0.5 * tf.reduce_sum(input_tensor=- tf.math.log(self.l_post_v + 1e-8)  \
                                         + self.l_post_v/local_l_variance \
                                         + tf.square(self.l_post_m - local_l_mean)/local_l_variance  \
                                         + tf.math.log(local_l_variance + 1e-8) - 1, axis=1)

        kl_gauss_z = 0.5 * tf.reduce_sum(input_tensor=- tf.math.log(self.z_post_v + 1e-8) + self.z_post_v + tf.square(self.z_post_m) - 1, axis=1)
        
        # Evidence lower bound        
        self.ELBO_gauss = tf.reduce_mean(input_tensor=recon_loss - kl_gauss_l - self.kl_scale * kl_gauss_z) 
        self.autoencoder_loss = - self.ELBO_gauss                                
              
        # Discriminator D1        
        self.dis_loss = - tf.math.log(tf.reduce_sum(input_tensor=tf.sqrt(tf.abs(self.dis_real_logit/tf.reduce_sum(input_tensor=self.dis_real_logit)
                                            * self.dis_fake_logit/tf.reduce_sum(input_tensor=self.dis_fake_logit)) )) + 1e-10)        
                
        # Discriminator D2 
        self.dis2_loss = - tf.math.log(tf.reduce_sum(input_tensor=tf.sqrt(tf.abs(self.dis2_real_logit/tf.reduce_sum(input_tensor=self.dis2_real_logit)
                                            * self.dis2_fake_logit/tf.reduce_sum(input_tensor=self.dis2_fake_logit)) )) + 1e-10)
        
        # Generator loss
        self.generator_loss = - tf.math.log(tf.reduce_sum(input_tensor=tf.sqrt(tf.abs(
                                            self.dis_fake_logit/tf.reduce_sum(input_tensor=self.dis_fake_logit)))) + 1e-10)
    ...
    def train_cluster(self):
        ...
        autoencoder_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr,
                                                       beta1=self.beta1).minimize(self.autoencoder_loss)
        
        discriminator_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr,
                                                         beta1=self.beta1).minimize(self.dis_loss,
                                                                                      var_list=self.dis_vars)
        generator_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr,
                                                     beta1=self.beta1).minimize(self.generator_loss,
                                                                                  var_list=self.gen_vars)
        # Discriminator D2
        discriminator2_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr,
    ...
    def encoder(self, x, reuse=False):
        ...
        with tf.compat.v1.variable_scope('Encoder') as scope:
            ...
            if self.is_bn:
                h = tf.compat.v1.layers.batch_normalization(
                ...
                for i in range(1, self.num_layers):
                    ...
                    h = tf.nn.dropout(lrelu(dense(h, self.g_h_dim[i - 1], self.g_h_dim[i], name='enc_h' + str(i) + '_lin'),
                              alpha=self.leak), rate=1 - (self.keep_prob))                    
    ...
    def discriminator2(self, z, z_dim, reuse=False):    
        ...
        :return: tensor of shape [batch_size, 1]
        """
        with tf.compat.v1.variable_scope('Discriminator2') as scope:
            if reuse:
                scope.reuse_variables()
    ...
    def sample_gaussian(self, mean, variance, scope=None):
        ...
        with tf.compat.v1.variable_scope(scope, 'sample_gaussian'):
            sample = tf.random.normal(tf.shape(input=mean), mean, tf.sqrt(variance))
    ...
    def zinb_model(self, x, mean, inverse_dispersion, logit, eps=1e-8): 
        ...
        return tf.reduce_sum(input_tensor=expr, axis=-1)
